...
const [currentVersion, setCurrentVersion] = useState('sub');
  const [newEpisodeId, setNewEpisodeId] = useState(null);
...
const currentEpisodeIndex = episodes
  ? episodes.findIndex(({ id }) => id === (newEpisodeId || episodeId))
  : -1;

const nextEpisodeId =
  currentEpisodeIndex !== -1 && currentEpisodeIndex + 1 < episodes.length
    ? episodes[currentEpisodeIndex + 1].id
    : null;

const previousEpisodeId =
  currentEpisodeIndex !== -1 && currentEpisodeIndex - 1 >= 0
    ? episodes[currentEpisodeIndex - 1].id
    : null;
...
const handleDub = async () => {
  ...
  console.log("Calling getVideo with newEpisodeId:", newEpisodeId);
    ...
    window.history.replaceState(null, '', newUrl);
        console.log("Calling getVideo with newEpisodeId:", newEpisodeId);
  setNewEpisodeId(newEpisodeId);

    // Call getVideo with the updated episodeId
...
const handleEpisodeClick = (episodeId) => {
    // Update newEpisodeId when an episode is clicked
    setNewEpisodeId(episodeId);
  };

const handleEpisodeClickInList = (episodeId) => {
   setNewEpisodeId(episodeId);
  ...
  try {
    console.log('Fetching data for episode ID:', episodeId);
    ...
    setLoading(false);
      ...
      const getVideo = async (customEpisodeId) => {
    setLoading(true);

    try {
      const apiUrl = customEpisodeId
        ? `https://api-amvstrm.nyt92.eu.org/api/v2/stream/${customEpisodeId}`
        : `https://api-amvstrm.nyt92.eu.org/api/v2/stream/${episodeIdRef.current}`;
    ...
    const usedEpisodeId = customEpisodeId || episodeIdRef.current;
    console.log(`https://api-amvstrm.nyt92.eu.org/api/v2/stream/${usedEpisodeId}`);
    setLoading(false);
  };
...
const DEFAULT_API_ENDPOINT = "https://juanito66.vercel.app/meta/anilist/episodes";
...
// UseEffect for fetching video data (runs once on mount)
useEffect(() => {
  let isMounted = true;
 ...
 useEffect(() => {
    console.log("newEpisodeId:", newEpisodeId);
  }, [newEpisodeId]);
...
;
 ...
 {detail3.dub && (
  <> 
     <Button onClick={handleDub} disabled={isButtonDisabled}>
    Watch Dub
  </Button>
  ...
  Watch Sub
        ...
        {episodes && (
          <>
            {episodes.map((episode) => {
              const isNewOrCurrentEpisodeId = episode.id === (newEpisodeId || episodeId);

              if (isNewOrCurrentEpisodeId) {
   ...
   <EpisodeList>
        {previousEpisodeId && (
          <EpisodeLink
            to={`/meta/anilist/watch/${previousEpisodeId}`}
            state={{ id: `${id}` }}
            onClick={() => handleEpisodeClickInList(previousEpisodeId)}
          >
            Previous Episode
          </EpisodeLink>
        )}
        ...
        {nextEpisodeId && (
          <EpisodeLink
            to={`/meta/anilist/watch/${nextEpisodeId}`}
            state={{ id: `${id}` }}
            onClick={() => handleEpisodeClickInList(nextEpisodeId)}
          >
            Next Episode
          </EpisodeLink>
        )}
